...
('ZPAF23S021','2023-04-19','2023-05-03','2023-05-10'),
...
select fg.PatientID,
       fg.StudyDate,
            group_concat(CASE WHEN ImageType = 'pre' THEN fg.SeriesDescription       ELSE NULL END)  PREDescription,
                   count(CASE WHEN ImageType = 'pre' THEN fg.SeriesDescription       ELSE NULL END)  PRECount,
            group_concat(CASE WHEN ImageType = 'art' THEN fg.SeriesDescription       ELSE NULL END)  ARTDescription,
                   count(CASE WHEN ImageType = 'art' THEN fg.SeriesDescription       ELSE NULL END)  ARTCount,
            group_concat(CASE WHEN ImageType = 'ven' THEN fg.SeriesDescription       ELSE NULL END)  VENDescription,
                   count(CASE WHEN ImageType = 'ven' THEN fg.SeriesDescription       ELSE NULL END)  VENCount,
            group_concat(CASE WHEN ImageType = 'cta' THEN fg.SeriesDescription       ELSE NULL END)  CTACount,
                   count(CASE WHEN ImageType = 'cta' THEN fg.SeriesDescription       ELSE NULL END)  CTACount,
       fg.StudyInstanceUID,
