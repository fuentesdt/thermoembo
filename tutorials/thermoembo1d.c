...
typedef enum {PARAM_OMEGA,
              ...
              PARAM_ARTIFICIALDIFFUSION,
              PARAM_VESSEL_COUPLING} ParameterType;
typedef struct {
  PetscInt          dim;
  PetscInt          refine;
...
static PetscInt nearest1d(const PetscReal x[], const AppCtx *ctx)
{
  PetscInt  kmin = 0;
  PetscReal d2min = PETSC_MAX_REAL;
  for (PetscInt k = 0; k < ctx->n1d; k++) {
    PetscReal dx = x[0] - ctx->xyz1d_m[3*k];
    PetscReal dy = x[1] - ctx->xyz1d_m[3*k+1];
    PetscReal dz = x[2] - ctx->xyz1d_m[3*k+2];
...
static PetscErrorCode p1d_func(PetscInt dim, PetscReal time, const PetscReal x[], PetscInt Nc, PetscScalar *u, void *ctx)
{
  AppCtx *options = (AppCtx *)ctx;
  if (options->n1d <= 0) { *u = options->parameters[PARAM_BASELINEPRESSURE]; return 0; }
  *u = options->p1d_atm[nearest1d(x, options)];
  return 0;
}

static PetscErrorCode gv_func(PetscInt dim, PetscReal time, const PetscReal x[], PetscInt Nc, PetscScalar *u, void *ctx)
{
  AppCtx   *options = (AppCtx *)ctx;
  if (options->n1d <= 0) { *u = 0.0; return 0; }
  PetscInt  k  = nearest1d(x, options);
  PetscReal dx = x[0] - options->xyz1d_m[3*k];
  PetscReal dy = x[1] - options->xyz1d_m[3*k+1];
  PetscReal dz = x[2] - options->xyz1d_m[3*k+2];
  PetscReal d2 = dx*dx + dy*dy + dz*dz;
  PetscReal sigma = options->r1d_m[k];
...
static PetscErrorCode ProcessOptions(MPI_Comm comm, AppCtx *options)
{
  ...
  options->parameters[PARAM_VESSEL_COUPLING] = 0.0;
  ierr = PetscOptionsReal("-vesselcoupling", "vessel-tissue coupling conductance amplitude [1/s/atm]", "thermoembo1d.c", options->parameters[PARAM_VESSEL_COUPLING], &options->parameters[PARAM_VESSEL_COUPLING], NULL);CHKERRQ(ierr);
  {
    ...
    if (vtpflg) {
      ...
      vtpreader->Update();
      vtkPolyData  *pd   = vtpreader->GetOutput();
      vtkIdType     npts = pd->GetNumberOfPoints();
      options->n1d = (PetscInt)npts;
      ierr = PetscMalloc1(3*npts, &options->xyz1d_m);CHKERRQ(ierr);
      ierr = PetscMalloc1(  npts, &options->p1d_atm);CHKERRQ(ierr);
      ierr = PetscMalloc1(  npts, &options->r1d_m  );CHKERRQ(ierr);
      double coord[3];
      for (vtkIdType k = 0; k < npts; k++) {
        pd->GetPoint(k, coord);
        options->xyz1d_m[3*k]   = coord[0] * 1.e-3;
        options->xyz1d_m[3*k+1] = coord[1] * 1.e-3;
        options->xyz1d_m[3*k+2] = coord[2] * 1.e-3;
      }
      vtkDataArray *parr = pd->GetPointData()->GetArray("pressure_mmhg");
      vtkDataArray *rarr = pd->GetPointData()->GetArray("radius_mm");
      for (vtkIdType k = 0; k < npts; k++) {
...
static PetscErrorCode Setup1DAux(DM dm, AppCtx *ctx)
{
  DM              dmAux;
  ...
  /* Reuse quadrature from the primary pressure FE */
  ...
  /* Attach to the primary DM so FEM integration sees the aux fields */
  ierr = PetscObjectCompose((PetscObject)dm, "dmAux", (PetscObject)dmAux);CHKERRQ(ierr);
  ierr = PetscObjectCompose((PetscObject)dm, "A",     (PetscObject)aux);CHKERRQ(ierr);

  /* PetscObjectCompose takes a reference; safe to destroy local handles */
